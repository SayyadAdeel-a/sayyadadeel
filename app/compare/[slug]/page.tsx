...
export async function generateMetadata({
  ...
  const item = comparisons.find((c) => c.slug === slug) as Comparison | undefined;
  if (!item) return {};
  return {
    ...
    alternates: {
      canonical: `https://adeelsayyad.tech/compare/${item.slug}`,
    },
  };
}
...
export default async function ComparisonPage({
  ...
  const relatedItems = comparisons.filter(
    (c) => item.relatedTerms?.includes(c.slug) && c.slug !== item.slug
  ).slice(0, 3);
      ...
      <main className="min-h-screen bg-[#f5f5f2]">
        <article className="mx-auto max-w-3xl px-6 py-12 pb-24">
          ...
          <nav className="mb-8 font-tight text-[13px] text-black/40">
            ...
            <Link href="/compare" className="hover:text-[#121212]">Compare</Link>
            <span className="mx-2">/</span>
          ...
          <div className="relative mb-6 inline-flex items-center gap-2 border border-dashed border-black/10 px-4 py-2">
            ...
            <span className="font-lato text-[13px] font-bold tracking-[-0.39px] text-[#121212]">Comparison</span>
          </div>
          ...
          <div className="mb-12 grid gap-6 md:grid-cols-2">
            {/* FieldOS */}
            <section className="rounded-[16px] border border-black/[0.06] bg-white p-6">
              <div className="mb-4 flex items-center gap-2">
                <span className="font-lato text-[13px] font-bold uppercase tracking-[0.5px] text-[#121212]">
                  FieldOS
                </span>
              </div>
              <h3 className="mb-3 font-instrument-serif text-[18px] tracking-[-0.54px] text-[#121212]">
                Strengths
              </h3>
              <ul className="mb-4 flex flex-col gap-2">
                {item.comparison.fieldOS.strengths.map((s, i) => (
                  <li key={i} className="flex items-start gap-2 text-[14px] leading-[1.5] text-black/60">
                    <span className="mt-1 size-[5px] shrink-0 rounded-full bg-[#c8ff00]" />
                    {s}
                  </li>
                ))}
              </ul>
              <p className="text-[13px] leading-[1.5] text-black/40 italic">
                Best for: {item.comparison.fieldOS.bestFor}
              </p>
            </section>
            ...
            <section className="rounded-[16px] border border-black/[0.06] bg-white p-6">
              <div className="mb-4 flex items-center gap-2">
                <span className="font-lato text-[13px] font-bold uppercase tracking-[0.5px] text-black/40">
                  Competitor
                </span>
              </div>
              <h3 className="mb-3 font-instrument-serif text-[18px] tracking-[-0.54px] text-[#121212]">
              ...
              <ul className="mb-4 flex flex-col gap-2">
                {item.comparison.competitor.strengths.map((s, i) => (
                  <li key={i} className="flex items-start gap-2 text-[14px] leading-[1.5] text-black/60">
                    <span className="mt-1 size-[5px] shrink-0 rounded-full bg-black/20" />
                    {s}
                  </li>
                ))}
              </ul>
              <p className="text-[13px] leading-[1.5] text-black/40 italic">
                Best for: {item.comparison.competitor.bestFor}
              </p>
            </section>
          </div>
