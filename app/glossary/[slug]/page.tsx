...
export async function generateStaticParams() {
  return glossary.map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata({
  ...
  const item = glossary.find((g) => g.slug === slug) as GlossaryItem | undefined;
  if (!item) return {};
  return {
...
export default async function GlossaryPage({
  ...
  const relatedItems = glossary.filter(
    (g) => item.relatedTerms?.includes(g.slug) && g.slug !== item.slug
  ).slice(0, 3);
      ...
      <main className="min-h-screen bg-[#f5f5f2]">
        ...
        <article className="mx-auto max-w-3xl px-6 pb-24">
          ...
          <nav className="mb-8 font-tight text-[13px] text-black/40">
            ...
            <Link href="/glossary" className="hover:text-[#121212]">Glossary</Link>
            <span className="mx-2">/</span>
          ...
          <section className="mb-12">
            ...
            <ul className="flex flex-col gap-3">
              {item.whatItInvolves.map((point, i) => (
          ...
          <section className="mb-12">
            ...
            <p className="text-[16px] leading-[1.7] text-black/60">
              {item.whyItMatters}
            </p>
          </section>
